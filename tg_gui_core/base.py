...
class Defaults():
    ...
    def __init__(self, *,
        margin, radius, font_size,
        _fill_color_,   _selected_fill_,
        _text_color_,   _selected_text_,
    ):
        self.margin     = margin
        self.radius     = radius
...
class Screen():
    ...
    def __getattr__(self, name):
        ...
        if self.outer is not None:
            try:    return getattr(self.outer, name)
            except: pass
...
class Widget():
    ...
    def __init__(self, *, superior=None, margin=None): # TODO: should use nest or superior kwarg?
        global Widget
        self._id_ = uid()
    ...
    def isnestedin(self, maybe_superior):
        return ( self._superior_ is maybe_superior
    ...
    def _place_(self, coord, dims):
        ...
        if self._superior_ is None and (x < 0 or y < 0): raise ValueError(
            f"right aligned coord cannot be used with root widgets"
        )

        # adjust
        if x < 0: x = self._superior_.width - width + 1 + x
        if y < 0: y = self._superior_.height - height + 1 + y

        # save
        self._placement_ = placement = x, y, w, h = (x, y, width, height)

        # calc relative placement
        self._rel_placement_ = rel_placement = (mar+x, mar+y, w-(2*mar), h-(2*mar))
